...
export class DocumentManager {
  ...
  constructor(adapter: StorageAdapter) { this.adapter = adapter; }

  async init(): Promise<void> {
    const existing = await this.adapter.getManifest();
    if (existing) { this.manifest = existing; }
    else { this.manifest = createEmptyManifest(); await this.persistManifest(); }
    const savedActive = localStorage.getItem(DOC_CONSTANTS.ACTIVE_DOC_KEY);
    if (savedActive && this.manifest.documents[savedActive]) { this.activeDocId = savedActive; }
  }

  getManifest(): Manifest { return this.manifest; }
  getActiveDocumentId(): string | null { return this.activeDocId; }

  setActiveDocument(id: string): void {
    if (!this.manifest.documents[id]) throw new Error(`Document ${id} not found`);
    this.activeDocId = id;
  ...
  async createDocument(name?: string): Promise<DocumentMeta> {
    const id = generateId();
    const now = Date.now();
    const meta: DocumentMeta = { id, name: name || DOC_CONSTANTS.DEFAULT_DOC_NAME, folderId: DOC_CONSTANTS.ROOT_FOLDER_ID, createdAt: now, updatedAt: now, version: 1, remoteVersion: null, dirty: false };
  ...
  async loadDocumentData(id: string): Promise<DocumentData | null> { return this.adapter.loadDocument(id); }
  ...
  async deleteDocument(id: string): Promise<void> {
    ...
    if (folder) { folder.documents = folder.documents.filter((dId) => dId !== id); }
    delete this.manifest.documents[id];
    this.manifest.version += 1;
    await this.adapter.deleteDocument(id);
    await this.persistManifest();
    if (this.activeDocId === id) { this.activeDocId = null; localStorage.removeItem(DOC_CONSTANTS.ACTIVE_DOC_KEY); }
  }
  ...
  async duplicateDocument(id: string): Promise<DocumentMeta> {
    ...
    if (data) { await this.moveDocument(newMeta.id, original.folderId); await this.adapter.saveDocument(newMeta.id, data, this.manifest.documents[newMeta.id]); }
  ...
  async moveDocument(docId: string, targetFolderId: string): Promise<void> {
    ...
    if (oldFolder) { oldFolder.documents = oldFolder.documents.filter((id) => id !== docId); }
    if (!targetFolder.documents.includes(docId)) { targetFolder.documents.push(docId); }
  ...
  async createFolder(name: string, parentId?: string): Promise<FolderNode> {
    ...
    const folder: FolderNode = { id, name, parentId: effectiveParentId, children: [], documents: [] };
  ...
  async deleteFolder(id: string): Promise<void> {
    const folder = this.manifest.folders[id];
    if (!folder || id === "root") return;
    for (const childId of folder.children) { await this.deleteFolder(childId); }
    for (const docId of folder.documents) { await this.deleteDocument(docId); }
    if (folder.parentId) {
  ...
  getDirtyDocuments(): DocumentMeta[] { return Object.values(this.manifest.documents).filter((d) => d.dirty); }
  getDocumentsInFolder(folderId: string): DocumentMeta[] {
    const folder = this.manifest.folders[folderId];
    if (!folder) return [];
    return folder.documents.map((id) => this.manifest.documents[id]).filter(Boolean);
  }

  private async persistManifest(): Promise<void> { await this.adapter.saveManifest(this.manifest); }
}
