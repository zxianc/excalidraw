...
export const SettingsDialog: React.FC<SettingsDialogProps> = ({
  ...
  const loadExisting = useCallback(() => {
    ...
    if (saved) {
      try {
        const c: SyncConfig = JSON.parse(saved);
        setForm({
          endpoint: c.endpoint || "",
          bucket: c.bucket || "",
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const a = new S3Adapter(buildConfig());
  ...
  const handleSave = () => {
    const c = buildConfig();
    localStorage.setItem(DOC_CONSTANTS.SYNC_CONFIG_KEY, JSON.stringify(c));
    onConfigSaved(c);
    onClose();
  };
  const handleClear = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      const e = await ConfigCrypto.encrypt(buildConfig(), password);
      setExportStr(e);
      setShowExport(true);
    } catch (e: any) {
  ...
  const handleImport = async () => {
    if (!importStr || !password) {
      alert("Paste config and enter password");
      return;
    }
    try {
      const c = await ConfigCrypto.decrypt(importStr, password);
      setForm({
        endpoint: c.endpoint,
        bucket: c.bucket,
        accessKey: c.accessKey,
      ...
      alert(`Import failed: ${e.message || "Wrong password"}`);
    }
  };
        ...
        <div className="settings-dialog__tabs">
          <button
            className={`settings-dialog__tab ${
              tab === "sync" ? "settings-dialog__tab--active" : ""
            }`}
            onClick={() => setTab("sync")}
          >
            Cloud Sync
          ...
          <button
            className={`settings-dialog__tab ${
              tab === "local" ? "settings-dialog__tab--active" : ""
            }`}
        ...
        <div className="settings-dialog__body">
          {tab === "sync" && (
            <div className="settings-dialog__sync">
              <div className="settings-dialog__field">
                <label>Endpoint URL (COS 域名)</label>
                <input
                  type="url"
              ...
              <div className="settings-dialog__field">
                <label>Bucket Name (存储桶名)</label>
                <input
                  type="text"
                  placeholder="exclidarw-1252379480"
                  value={form.bucket}
                  onChange={(e) => setForm({ ...form, bucket: e.target.value })}
                />
              </div>
              <div className="settings-dialog__field">
                <label>SecretId (访问密钥 ID)</label>
                <input
                  type="text"
                  placeholder="AKID..."
              ...
              <div className="settings-dialog__field">
                <label>SecretKey (访问密钥 Key)</label>
                <input
                  type="password"
                  placeholder="••••••••"
                ...
                {showImport && (
                  <div className="settings-dialog__import-form">
                    ...
                    <textarea
                      ...
                      placeholder="Paste encrypted config..."
                    />
                    <button
                      className="settings-dialog__btn settings-dialog__btn--primary"
          ...
          {tab === "local" && (
            <div className="settings-dialog__local">
              ...
              <p>
                Documents are stored in IndexedDB and automatically synced when
                online.
              </p>
