...
**Purpose:** Enable users to securely share S3 cloud sync configuration across devices by encrypting credentials with a user-chosen password (AES-GCM via Web Crypto API), exporting as base64 text and QR code, and importing via paste or camera scan — all without a backend server.
